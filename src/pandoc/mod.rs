...
pub fn create_pandoc_config(data: &Document, input: &str) -> Vec<String> {
    match &data.document_type {
        DocumentType::PDF => create_pandoc_args(&data.clone(), input, Some(vec!["--toc"])),
        DocumentType::PRESENTATION => create_pandoc_args(
            &data.clone(),
            input,
            Some(vec![
                "-t",
    ...
    if input.ends_with(".yml") {
        config.append(
            &mut data
                .inputs
                .iter()
                .filter(|input| input.ends_with(".md"))
                .map(|x| x.as_ref())
                .collect(),
        );
...
mod tests {
    ...
    fn creating_pandoc_args_no_bib() {
        ...
        assert_eq!(
            pandoc_args,
            vec!("-s", "md.md", "pdf.yml", "-o", "pdf/no_filename.pdf")
        )
    }
